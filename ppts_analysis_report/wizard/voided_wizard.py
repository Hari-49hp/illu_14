...
class VoidedWizard(models.TransientModel):
    _name = 'voided.wizard'
    _description ="VoidedWizard"
    ...
    view = fields.Selection([('summary', 'Summary'),('detail', 'Detail')], string='View', default='summary')

    def action_apply(self):
        if self.view == 'summary':
            if self.company_id:
                action = self.env.ref('ppts_analysis_report.voided_sales_sumy_action_view').read()[0]
            ...
            if self.company_id:
                action = self.env.ref('ppts_analysis_report.sales_voided_detail_action_view').read()[0]
