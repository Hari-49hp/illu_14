    ...
    toggleIsToInvoice() {
        ...
        async validateOrder(isForceValidate) {
        	if (await this._isOrderValid(isForceValidate)) {
                ...
                var sess_id_set = this['env']['pos']['config']['id']
                $.ajax({
                	url: "/pos/whatsapp/message/automatic",
                	type: 'POST',
                	async:false,
                	data: {
                		"session_req_id":this['env']['pos']['config']['id'],
                		"partner_id":this['env']['pos']['attributes']['selectedClient']['id'],
                	},
                	success: function(result) {
                		if (result === 'no_mobile'){
                			$.MessageBox("Customer mobile number empty");
                		}
                		else if (result === 'success'){
                			$.MessageBox("Message Send Successful").done(function(data, button){
                				setTimeout(function(){
                					$("body > div.o_action_manager > div > div.pos-content > div > div > div > div > div > div > div.default-view > div.actions > form > div.input-email > button").click();

                                    console.log(sess_id_set);

                                    $.ajax({
                                        url: "/pos/whatsapp/message/order_enable",
                                        type: 'POST',
                                        async:false,
                                        data: {
                                            "session_req_id":sess_id_set,
                                        },
                                        success: function(result) {
                                            if(result === 'false'){
                                                $('#new-order-btn-pos').show();
                                                $('#home-complete-btn-pos').hide();
                                            }
                                            else{
                                                $('#new-order-btn-pos').hide();
                                                $('#home-complete-btn-pos').show();
                                            }
                                        },
                                    });
