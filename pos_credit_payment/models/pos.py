...
class PosConfig(models.Model):
    ...
    invoice_credit_payment = fields.Selection(
        [('full_amount', 'Full Amount(without credit)'), ('partial_amount', 'Partial Amount(with credit)')], string='',
        default='full_amount')
...
class pos_order(models.Model):
    ...
    def action_pos_order_invoice(self):
        ...
        for order in self:
            ...
            if invoice_credit_payment != 'partial_amount':
                return super(pos_order, self).action_pos_order_invoice()
            else:
    ...
    @api.model
    def UpdateCreditJournal(self, order, clients, company_id, pos_cur, amount):

        AccountMove = self.env['account.move']
        partner = self.env['res.partner'].search([('id', '=', clients['id'])])
        journel_id = self.env['account.journal'].search([('type', '=', 'cash'), ('company_id', '=', company_id[0])],
                                                        limit=1)
        move =False
        if amount > 0:
            pass
    ...
    @api.model
    def updateReturnReason(self, name, note):
