...
class EventEvent(models.Model):
    ...
    room_id = fields.Many2one('roomtype.master',string="Room",tracking=True)
    facilitator_evnt_ids=fields.Many2many('hr.employee',string='Facilitator ',help='List of Facilitators Name',domain="[('company_id','=',company_id)]")


class MultiDateWizard(models.Model):
    _inherit = "multi.date.wizard"
    room_id = fields.Many2one('roomtype.master',string="Room",tracking=True)
...
class EventMeetingRoom(models.Model):
    ...
    room_max_capacity = fields.Selection(string="Max capacity", related="chat_room_id.max_capacity", readonly=False,
                                         required=True)
    booking_types = fields.Selection([
    ...
    therapist_id = fields.Many2one('hr.employee',string='Therapist',related='event_id.eve_therapist_id',help='Event Therpist Name')
    # many2many added as per client requirement
    therapist_ids = fields.Many2many('hr.employee',string='Facilitator',related='event_id.facilitator_evnt_ids',help='List of Facilitator of Event')
    room_categ_id = fields.Many2one('room.maincateg.master',string='Serivce Category',help='Room Service Category')
    event_start_dt = fields.Datetime(string='Start Date',related='event_id.date_begin',help='Event Starting Date')
    event_end_dt = fields.Datetime(string='End Date',related='event_id.date_end',store=True,help='Event Ending Date')
    room_id = fields.Many2one('roomtype.master',string='Room',required=True,help='Name of Room')
    event_location_id = fields.Many2one('venue.venue',related='event_id.address_id',string='Location',required=False,help='Location of Event')
    room_incharge_id = fields.Many2one('res.users',string='House Keeping',help='Name of the House Keeping Person')
    reception_id = fields.Many2one('res.users',string='Receptionist',help='Name of the Receptionist')
    max_seats = fields.Integer(string='Max. Capacity',related='event_id.seats_max',help='Maximum limit of Event Attendees')
    duration = fields.Float('Duration',related='event_id.duration',readonly=True,help='Duration of the Event.')
    allday = fields.Boolean('All Day', default=False)
    description = fields.Text('Description')
    remainder = fields.Char('Remainder',help='Remainder info of Rooms')
    internal_note = fields.Text('Internal Note',help='Internal Notes about Rooms')
    ...
    # this method is commented because room created from event template
    @api.model
    def create(self, vals):
        rres = super(EventMeetingRoom, self).create(vals)
        for rec in rres:
            if rec.room_id:
                rec.event_id.write({
    ...
    #     query ="UPDATE event_event SET room_id = "+str(res.room_id.id)+" WHERE id="+str(res.event_id.id)+";"
    #     self.env.cr.execute(query)
        return rres

    def write(self, vals):
        res = super(EventMeetingRoom, self).write(vals)
        for rec in self:
            if rec.room_id:
                rec.event_id.write({
                              'room_id': rec.room_id.id,
    ...
    #             query ="UPDATE event_event SET room_id = "+str(rec.room_id.id)+" WHERE id="+str(rec.event_id.id)+";"
    #             self.env.cr.execute(query)
        return res

    @api.onchange('event_id','room_categ_id')
    def _compute_name_topic(self):
        if self.event_id:
            self.name=self.event_id.event_type_id.name
        self.room_max_capacity='no_limit'

    @api.onchange('event_id','room_id')
    def _eveent_room_control(self):
        for re in self:
            if re.room_id:
                room_exists =self.env['event.meeting.room'].search([('room_id', '=', re.room_id.id),
                                                                  ('event_start_dt', '<=', re.event_start_dt),
                                                                  ('event_end_dt', '>=', re.event_end_dt),])                
                if room_exists:
                    room_exists_first=room_exists[0].event_id.id
                    room_exists_firste=self.env['event.event'].search([('id', '=', room_exists_first)]).name
                    raise UserError('"' + re.room_id.room_type + '" is already assigned for '+room_exists_firste)
