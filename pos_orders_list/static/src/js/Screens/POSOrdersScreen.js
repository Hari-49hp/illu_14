odoo.define('pos_orders_list.POSOrdersScreen', function (require) {
	...
	class POSOrdersScreen extends PosComponent {
		constructor() {
			...
			this.updateOrderListDefault = debounce(this.updateOrderListDefault, 70);
		...
		get pos_orders() {
			...
			if (this.orders) {
				if ((query && query !== '') || (this.props.selected_partner_id)) {
		...
		refresh_orders() {
			...
			// this.env.pos['attributes']['selectedClient'] = null;
			console.log('refresh -->', this.props.selected_partner_id)
			this.state.query = '';
		...
		updateOrderList(event) {
			// this.env.pos['attributes']['selectedClient'] = null;
			// console.log('update order --> ', this.env.pos['attributes']['selectedClient'])
			...
			if (pos_config.pos_session_limit == 'all') {
				if (pos_config.show_draft == true) {
					if (pos_config.show_posted == true) {
						return [['state', 'in', ['draft', 'done','invoiced']]];
					}
					else {
		...
		async get_pos_orders() {
			let selected_partner_oid
			if (this.env.pos['attributes']['selectedClient'] === null) {
				selected_partner_oid = false
			}
			else {
				selected_partner_oid = parseInt(this.env.pos['attributes']['selectedClient']['id'])
			}
			...
			if (selected_partner_oid !== false && isNaN(selected_partner_oid) !== true) {
				pos_domain.push(['partner_id', '=', selected_partner_oid]);
			}
