odoo.define('pos_coupon.pos', function (require) {
    ...
    models.Order = models.Order.extend({
        ...
        initialize: function () {
            ...
            console.log("Order Update in Coupon Res",res);
            if (res === undefined)
                res = this
            res.on(
                'update-rewards',
...
//            _order_super.set_orderline_options.apply(this, [orderline, options]);
        ...
        /**
        add_product: function (line, options) {
            _order_super.add_product.apply(this, [line, options]);
            if (options && options.is_program_reward) {
                line.is_program_reward = true;
                line.tax_ids = options.tax_ids;
        ...
        **/
        ...
        resetPrograms: function () {
            ...
            Gui.showPopup('ErrorPopup', {
								'title': "Deactivate",
								'body': "Active coupons and promo codes were deactivated.",
							});
//            this.do_notify('Active coupons and promo codes were deactivated.');
            this.trigger('update-rewards');
        },
        /**
        ...
        activateCode: async function (code) {
            ...
            if (promoProgram && this.activePromoProgramIds.includes(promoProgram.id)) {
                Gui.showPopup('ErrorPopup', {
								'title': "Error",
								'body': "That promo code program has already been activated..",
							});
//                this.do_notify('That promo code program has already been activated.');
                ...
                Gui.showPopup('ErrorPopup', {
								'title': "Error",
								'body': "That coupon code has already been scanned and activated..",
							});
//                this.do_notify('That coupon code has already been scanned and activated.');
            } else {
...
//                    this.do_notify(payload.error_message);
                }
            }
        },
        /**
        ...
        set_quantity: function (quantity, keep_price) {
            ...
            if (quantity === 'remove' && this.is_program_reward) {
                ...
                if (related_rewards.length !== 0) {
                    Gui.showPopup('ReturnWidget', {
								'title': "Removed",
								'body': "Other reward lines from the same program were also removed.",
							});
                }
            }
        },
