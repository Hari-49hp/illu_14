...
class Partner(models.Model):
    ...
    @api.model
    def call_originate_extension(self, partner_id):
        partner = self.browse(partner_id)
        ...
        # Now strip # and have only extension.
        dtmf_digits = extension.strip('#')
        self.env['asterisk_common.user'].originate_call_partner(
            number, model='res.partner', res_id=partner.id,variables={'__dtmf_digits': dtmf_digits,
                       '__dtmf_delay': dtmf_delay})
